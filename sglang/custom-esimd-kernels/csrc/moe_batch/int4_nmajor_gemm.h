...
{
    // gather() takes uint32 byte offsets. Offsets are strided by hidden_size on
    // the input side and by intermediate_size on the output and down-projection
    // sides, so the larger of the two bounds the range.
    {
    ...
    return q.submit([&](sycl::handler& cgh) {
        cgh.parallel_for(
            ...
            for (int m_base = t0; m_base < t1; m_base += MAX_M) {
                ...
                // carries duplicate lanes. This accumulate is a read-modify-write,
                // not an atomic, so duplicates would collide.
                simd<uint32_t, MAX_M> lane_id(0u, 1u);
                // The mask must bound the lane against the row count; these
                // copies are const and read once, immediately before it.
