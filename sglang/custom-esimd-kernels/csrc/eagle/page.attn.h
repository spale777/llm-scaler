...
#pragma unroll
    ...
    if ((flag & 0x1) == 1) {
      // Phase3 guards the identical reciprocal and says why (an empty batch
      // contributes no exp() terms, so the sum stays 0 and 1/0 makes every
      // output lane NaN). Phase3 only runs when maxKvSeqLen > 1024, so below
      // that this is the ONLY reciprocal on the path and the NaN is stored.
