...
/* Masked-tail variant: any remainder, not just the {8,16,32,64,128} residues
 * VL_TAIL requires. The residue loads under a lane predicate, so out-of-range
 * lanes contribute a hard zero and nothing past the row is dereferenced. */
...
struct GEMV_fp8_pert_bmg_masked_tail_kernel {
    ...
    void operator()(sycl::nd_item<1> item) const SYCL_ESIMD_KERNEL {
        ...
        // Gather VL_BIG lanes with only the first `tail` enabled: disabled
        // lanes yield 0 and no address past ks+kp is dereferenced.
        if (tail > 0) {
    ...
    uint32_t N, uint32_t K, int fp8_mode, sycl::queue& q)
{
    ...
    if (vl_tail < 0) {
        ...
        // Every (vl_big, vl_tail, ks) the selector can emit needs an arm: the
        // masked catch-all is exact but runs at width 32 regardless, so a
        // missing arm costs up to 8x.
