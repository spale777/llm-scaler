...
| Custom IPC all-reduce | ✅ | ❌ | ⚠️ | Handle exchange implemented on `sycl_ext_oneapi_inter_process_communication`, which the pinned compiler ships: portable byte payload, no raw `ze_api.h`, no `-lze_loader`. Peers map against the current stream's context; the probe requires `atomics_supported` over every ordered pair; `dispose_ctx` unmaps. Still not selected at runtime (`ca_comm = None`), and **never executed** -- the exchange needs two ranks. See §3 |
...
| ~~K8~~ | — | `.../fp8_moe_gemm_blockscale.h` | **FIXED.** `block_n` reaches the kernel and 32 is accepted; `block_k` was still pinned by `constexpr int BK = 128` in the decode launcher while the host took it as a parameter, so a narrower K block was scaled as though it were 128. BK is now a template parameter dispatched from the runtime value, with 32, 64 and 128 instantiated, and the vector width stays a whole number of scale blocks. DeepSeek V4.1's 32-wide block works. |
